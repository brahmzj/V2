...
const game = {
  ...
  ascensionPerks: {}
  ,
  // Player profile values.  These are set during the initial setup modal
  // and persist across sessions.  They are used to personalise the game
  // experience (e.g. displaying the cultivator's name or gender‑specific
  // pronouns where appropriate).  Default to empty strings and prompt
  // the player on first launch.
  playerName: '',
  playerGender: ''
  ,settings: {
    colorBlind: false,
...
/*
 * Avatar images for disciples.  We provide a small set of chibi faces that
 * are assigned to each disciple when they are recruited.  To increase variety
 * the base set has been expanded to include additional artwork (15–28) stored
 * in assets/icons/disciples.  When the base array is exhausted the hue
 * rotation logic later in the file will create further variations.
 */
const discipleAvatars = [
  ...
  'assets/icons/disciples/avatar15.png',
  'assets/icons/disciples/avatar16.png',
  'assets/icons/disciples/avatar17.png',
  'assets/icons/disciples/avatar18.png',
  'assets/icons/disciples/avatar19.png',
  'assets/icons/disciples/avatar20.png',
  'assets/icons/disciples/avatar21.png',
  'assets/icons/disciples/avatar22.png',
  'assets/icons/disciples/avatar23.png',
  'assets/icons/disciples/avatar24.png',
  'assets/icons/disciples/avatar25.png',
  'assets/icons/disciples/avatar26.png',
  'assets/icons/disciples/avatar27.png',
  'assets/icons/disciples/avatar28.png'
];
...
// Update the developer multiplier labels to display "locked" indicator in the
// currently selected language.  Because translations may be cached or
// unavailable, we provide our own fallback translations for each supported
// language.  This helper removes any existing text nodes within the
// checkbox labels and appends the translated text after the input element.
function updateDevToggleLabels() {
  try {
    const lang = (game.settings && game.settings.language) || localStorage.getItem('gameLang') || 'en';
    const lockTranslations = {
      en: { tap: 'Qi per tap ×100 (locked)', ps: 'Qi per second ×100 (locked)' },
      fr: { tap: 'Qi par tap ×100 (verrouillé)', ps: 'Qi par seconde ×100 (verrouillé)' },
      es: { tap: 'Qi por toque ×100 (bloqueado)', ps: 'Qi por segundo ×100 (bloqueado)' },
      zh: { tap: '每次点击气 ×100（锁定）', ps: '每秒气 ×100（锁定）' },
      hi: { tap: 'प्रत्येक टैप पर क्यूई ×100 (लॉक)', ps: 'प्रति सेकंड क्यूई ×100 (लॉक)' }
    };
    const tr = lockTranslations[lang] || lockTranslations.en;
    // Helper to update a single label given an input id and text
    const updateLabel = (inputId, text) => {
      const input = document.getElementById(inputId);
      if (!input) return;
      const label = input.parentElement;
      if (!label) return;
      // Remove all existing text nodes from the label
      const removeNodes = [];
      ...
      removeNodes.forEach(node => label.removeChild(node));
      // Append the translated text after the input
      const needsSpace = true;
      const textNode = document.createTextNode((needsSpace ? ' ' : '') + text);
      label.appendChild(textNode);
    };
    updateLabel('cb-tap-mult', tr.tap);
    updateLabel('cb-ps-mult', tr.ps);
  } catch (e) {
    // ignore errors
  }
}
...
function resetGame() {
  ...
  // Reset player profile fields.  Clearing these ensures that on the next launch
  // or after a reset the initial setup modal (language, gender and name)
  // appears again.  Without resetting these values, the modal would not
  // show because the setup check would detect existing values.
  game.playerName = '';
  game.playerGender = '';
  ...
  // After resetting the game and returning to the Story screen, prompt
  // the player again for their language, gender and name.  We do not
  // re‑run the entire init() routine here because that function also
  // performs other startup tasks that are not necessary during a reset
  // (e.g. attaching event listeners a second time).  Instead we
  // directly display the setup modal using the helper defined below.
  // A small delay gives the UI a chance to finish rendering the Story
  // page before showing the modal overlay.  Without the delay the
  // overlay could be hidden behind other elements.
  try {
    setTimeout(() => {
      // Always re‑initialise the game.  This sets up translations,
      // bindings and UI elements that may be needed before showing
      // the modal.  If init() is undefined for some reason, skip it.
      if (typeof init === 'function') {
        try {
          init();
        } catch (ignored) {
          // ignore errors from init; still attempt to show modal
        }
      }
      // After reinitialisation, display the setup modal.  This helper
      // ensures the modal exists and wires up a fresh confirm handler.
      if (typeof displaySetupModal === 'function') {
        displaySetupModal();
      }
    }, 50);
  } catch (ignored) {}
...
// When navigating to the story screen, refresh the chapters.  This ensures
  ...
  let handledBg = false;
    ...
    if (game.stage >= 1 && !game.sectName) {
      // Show a custom modal for entering the sect name.  This avoids using
      // the built‑in prompt() which does not match the game's visual style.
      showSectNameModal((name) => {
        // Assign chosen name or fallback
        game.sectName = name && name.trim() ? name.trim() : 'My Sect';
        saveGame();
        // Update the relevant sect header depending on the current screen
        try {
          if (id === 'sect-management') {
            ...
            if (header) header.textContent = `${game.sectName || 'Sect'}`;
          }
        } catch (ignored) {}
      });
    }
...
/**
 * Ensure the initial setup modal contains fields for language, gender and name.
 * This function modifies the DOM at runtime, adding radio buttons for gender
 * selection and a text input for the player's name if they do not already
 * exist.  It also updates the modal's heading and adds a short explanatory
 * paragraph.  Called from init() before showing the modal.
 */
function ensureSetupFields() {
  try {
    ...
    // For the initial setup we only ask for language.  Remove any gender/name
    // controls inserted by older versions and adjust the header accordingly.
    const header = modal.querySelector('h2');
    if (header) {
      header.textContent = 'Select your language';
    }
    // Remove explanatory text, gender fields and name input if they exist.
    const oldInstr = modal.querySelector('.setup-instructions');
    if (oldInstr) oldInstr.remove();
    const oldGender = modal.querySelector('.gender-select');
    if (oldGender) oldGender.remove();
    const oldName = modal.querySelector('#player-name-input');
    if (oldName) oldName.remove();
  } catch (err) {
    // fail silently; the modal may not exist yet during testing contexts
  }
}

/**
 * Ensure a modal exists for entering the player's sect name.  This modal
 * mirrors the styling of the language selection modal and will only be
 * created once.  It contains a header, a text input and a confirm button.
 */
function ensureSectNameModal() {
  if (document.getElementById('sect-name-modal')) return;
  const modal = document.createElement('div');
  modal.id = 'sect-name-modal';
  modal.className = 'modal hidden';
  // Build modal content
  const content = document.createElement('div');
  content.className = 'modal-content';
  const header = document.createElement('h2');
  header.textContent = 'Name Your Sect';
  const input = document.createElement('input');
  input.id = 'sect-name-input';
  input.type = 'text';
  input.placeholder = 'Enter your sect name';
  input.style.marginTop = '1rem';
  input.style.width = '100%';
  input.style.boxSizing = 'border-box';
  const confirm = document.createElement('button');
  confirm.id = 'sect-name-confirm';
...
/**
 * Ensure a modal exists for entering a password to unlock developer features.
 * The modal is styled similarly to other in‑game modals.  It contains a
 * header, a password input and confirm/cancel buttons.  This modal is
 * created only once and reused for both Qi per tap ×100 and Qi per second
 * ×100 toggles.  The modal remains hidden until explicitly shown via
 * showPasswordModal().
 */
...
/**
 * Display the password modal and call the provided callback with a boolean
 * indicating whether the entered password is correct.  The modal is
 * dismissed automatically after confirmation or cancellation.  The
 * password is case‑sensitive and must equal 'ottawa'.  If the user
 * cancels or enters an incorrect password, the callback receives false.
 *
 * @param {function(boolean):void} callback Called with true if password correct
 */
function showPasswordModal(callback) {
  ...
  const confirm = modal.querySelector('#password-confirm');
  const cancel = modal.querySelector('#password-cancel');
  // Reset input value each time
  ...
  const onConfirm = () => {
    ...
    const correct = entered === 'ottawa';
    if (!correct) {
      // Show a toast if available to indicate failure
      const msg = (typeof t === 'function') ? t('toast.invalidPassword') : 'Incorrect password';
      showToast(msg);
    }
    try {
      callback(correct);
...
/**
 * Display the initial setup modal (language, gender and name) immediately.
 * This helper ensures the modal fields exist and attaches a one‑time
 * confirmation handler that stores the selected values and hides the modal.
 * It is used on reset to prompt the player again without reloading the page.
 */
function displaySetupModal() {
  // Prepare fields (language dropdown, gender radios, name input)
  ensureSetupFields();
  ...
  if (!langModal || !langDropdown || !confirmBtn) return;
  // Reset dropdown default
  langDropdown.value = 'en';
  // No gender or name fields to clear anymore
  // Show modal
  langModal.classList.remove('hidden');
  // Attach a one‑off confirm handler.  Remove any previous handler by cloning
  // the node (quick way to remove old event listeners) then attach a new one.
  const newConfirm = confirmBtn.cloneNode(true);
  confirmBtn.parentNode.replaceChild(newConfirm, confirmBtn);
  newConfirm.id = 'lang-select-confirm';
  newConfirm.addEventListener('click', () => {
    ...
    // Update settings dropdown to reflect selection
    const settingsLangSelect = document.getElementById('language-select');
    if (settingsLangSelect) settingsLangSelect.value = selectedLang;
    // Save game state
    saveGame();
    // Optionally update stats UI after language change
    try {
      updateStatsUI();
    } catch (ignored) {}
  }, { once: true });
...
/**
 * Show the sect name modal and invoke the provided callback with the
 * entered name once the player confirms.  The modal will be hidden after
 * confirmation.  If the player leaves the input blank, a default name
 * "My Sect" will be used.  This helper allows asynchronous handling
 * without blocking the navigate() call.
 * @param {function(string):void} callback Function called with the sect name
 */
function showSectNameModal(callback) {
  ensureSectNameModal();
  const modal = document.getElementById('sect-name-modal');
  if (!modal) {
    callback('My Sect');
    return;
  }
  const input = modal.querySelector('#sect-name-input');
  const confirmBtn = modal.querySelector('#sect-name-confirm');
  ...
  const handler = () => {
    ...
    const name = input && input.value.trim() ? input.value.trim() : 'My Sect';
    modal.classList.add('hidden');
    try {
      callback(name);
    } catch (ignored) {}
...
function init() {
  ...
  // --- Override developer multiplier labels to show "locked" in all languages ---
  // Certain developer toggles (Qi per tap ×100 and Qi per second ×100) are
  // password‑protected and should be labelled as "locked" instead of "testing".
  // In some builds the translations may still reference the old testing label.
  // To ensure consistency we override these translation keys for all
  // supported languages here before any translations are applied.  If
  // additional languages are added, add corresponding entries below.
  try {
    const lockTranslations = {
      en: { tap: 'Qi per tap ×100 (locked)', ps: 'Qi per second ×100 (locked)' },
      fr: { tap: 'Qi par tap ×100 (verrouillé)', ps: 'Qi par seconde ×100 (verrouillé)' },
    ...
    if (typeof window.translations === 'object') {
      Object.keys(lockTranslations).forEach(lang => {
        const trans = window.translations[lang];
        if (trans) {
          trans['label.tapMult'] = lockTranslations[lang].tap;
          trans['label.psMult'] = lockTranslations[lang].ps;
        }
      });
    }
  } catch (e) {
    // ignore errors; translation overrides are optional
  }

  // --- Internationalisation / initial setup initialisation ---
  // Ensure the modal contains all required fields (language, gender and name)
  ensureSetupFields();
  ...
  try {
    ...
    // Determine if the initial language has been selected.  We no longer ask
    // for gender or name, so the setup modal appears only when the
    // language has not been chosen.  Once a language is set, it persists
    // through saves and reloads, and the modal will be skipped.
    const needsSetup = !langToUse;
    if (needsSetup) {
      if (langModal) {
        // Show modal overlay
        langModal.classList.remove('hidden');
        // Default selection to English if no language selected yet
        if (langDropdown && !langDropdown.value) langDropdown.value = 'en';
        if (langConfirmBtn) {
          langConfirmBtn.addEventListener('click', () => {
            // Retrieve selected language
            const selectedLang = langDropdown ? langDropdown.value : 'en';
            // Apply language via i18n helper; this also writes to localStorage
            if (typeof window.setLanguage === 'function') {
              window.setLanguage(selectedLang);
            }
            // Persist language to game state
            game.settings.language = selectedLang;
      ...
      // All required selections have been made previously; apply saved language immediately.
      if (typeof window.setLanguage === 'function') {
  ...
  // Password-protected developer multipliers
  // The following section manages the two developer toggles that allow
  // multiplying manual Qi gains and passive Qi gains by 100.  To avoid
  // accidental or unintended activation, a password is required when
  // turning these options on.  Turning them off always resets the
  // multipliers without prompting.  The password is case‑sensitive and
  // must be "ottawa".
  const cbTapMult = document.getElementById('cb-tap-mult');
  if (cbTapMult) {
    // Initialise checked state based on the saved multiplier
    cbTapMult.checked = !!(game.qiPerTapMult && game.qiPerTapMult > 1);
    // Ensure the element is enabled
    cbTapMult.disabled = false;
    // Apply multiplier and update UI/save
    const applyTap = () => {
      updateStatsUI();
      saveGame();
    };
    // Intercept clicks (capturing) on both the checkbox and its label to
    // control toggling.  Clicking the label naturally toggles the
    // associated checkbox; to intercept that behaviour we listen on the
    // parent label as well.  We prevent the default action and stop
    // propagation so no other handlers run.  When turning on, we prompt
    // for a password; turning off simply resets the multiplier.
    const handleTapToggle = (ev) => {
      ev.preventDefault();
      ev.stopImmediatePropagation();
      if (!cbTapMult.checked) {
        showPasswordModal((ok) => {
          if (ok) {
            cbTapMult.checked = true;
            game.qiPerTapMult = 100;
            applyTap();
          } else {
            cbTapMult.checked = false;
          }
        });
      } else {
        cbTapMult.checked = false;
    ...
    // Listen on the checkbox itself
    cbTapMult.addEventListener('click', handleTapToggle, true);
    // Listen on the parent label to catch clicks on the label text
    if (cbTapMult.parentElement) {
      cbTapMult.parentElement.addEventListener('click', handleTapToggle, true);
    }
  }
  ...
  if (cbPsMult) {
    ...
    cbPsMult.disabled = false;
    const applySec = () => {
      recalcProduction();
      updateStatsUI();
      saveGame();
    };
    const handlePsToggle = (ev) => {
      ev.preventDefault();
      ev.stopImmediatePropagation();
      if (!cbPsMult.checked) {
        showPasswordModal((ok) => {
          if (ok) {
            cbPsMult.checked = true;
            game.qiPerSecMult = 100;
            applySec();
          } else {
            cbPsMult.checked = false;
          }
        });
      } else {
        cbPsMult.checked = false;
    ...
    cbPsMult.addEventListener('click', handlePsToggle, true);
    if (cbPsMult.parentElement) {
      cbPsMult.parentElement.addEventListener('click', handlePsToggle, true);
    }
  }
  ...
  // After all UI elements are initialised and event handlers registered,
  // reapply translations.  Some labels (e.g. developer toggle labels) rely
  // on the translation strings defined in i18n.js and may not update
  // correctly unless applyTranslations() is invoked after dynamic
  // modifications.  Guard against missing function.
  try {
    if (typeof window.applyTranslations === 'function') {
      // Wrap applyTranslations to also update developer toggle labels whenever
      // translations are reapplied.  This ensures the locked labels stay
      // consistent across language changes.  Only wrap once to avoid
      // recursive wrapping.
      if (!window.__applyTranslationsWrapped) {
        const origApply = window.applyTranslations;
        window.applyTranslations = function() {
          origApply.apply(this, arguments);
          try {
            updateDevToggleLabels();
          } catch (e) {
            // ignore label update errors
          }
        };
        window.__applyTranslationsWrapped = true;
      }
      window.applyTranslations();
    }
    // Update developer toggle labels after translations to ensure
    // the correct "locked" indicator is displayed regardless of caching
    updateDevToggleLabels();
  } catch (e) {
    // ignore translation errors; the game will fall back to the default text
  }
